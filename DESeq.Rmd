...
for (i in resultsNames(dds)){ #loop and get results for all conditions
  if (i != 'Intercept'){
    ...
    # Get the top 15 rows with the greatest absolute values of log2FoldChange
    top_15_genes <- sig_genes %>%
    ...
    plotCounts(dds, gene = row.names(top_15_genes)[1], intgroup = 'Condition', normalized = TRUE) # plot the most differentiated gene in each condition against all conditions
    dev.off() #close pdf device
    all_genes[[i]]<-list(top15 =top_15_genes,res=res_shrunken_df)
